...
//                k=AES.keygeneration();
            ...
            String path = "C:\\Users\\Sushant\\Desktop\\encryptionandde\\";
...
//            println("convert")
            ...
            try {
                println(params)
                def key = rsaDEcrypt(params.user_key)
                println "key ===============================>>>>>>>>>>> $key"
...
//                fos.close();
    ...
    def RSAkeyencryption(){
        String key = params.key
        RSA rsa = new RSA(128);
        BigInteger plaintext = new BigInteger(key.getBytes());

        BigInteger ciphertext = rsa.encrypt(plaintext);
        String text1 = ciphertext.toString()
        println "text2 = $text1"
        render text1
    }
    def RSAkeydecryption(){
        String key = params.user_key
        String plaintext = rsaDEcrypt(key);
        render plaintext
    }

    String rsaDEcrypt(String key){
        RSA rsa = new RSA(128);
        String plaintext = rsa.decrypt(key);
        return plaintext
    }
